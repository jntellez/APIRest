...
@app.get("/eventos/estatus/{estatus}", tags=["Eventos"], summary="Consultar eventos por estatus")
def consultarEventosPorEstatus(estatus: str):
  return {"message": f"Eventos con estatus {estatus} consultados"}

@app.put("/eventos/estatus/{idEvento}", tags=["Eventos"], summary="Cambio de estatus de un evento")
